...
test("a protocol-relative value is rejected", () => {
  expect(sanitizeNext("//evil.com")).toBe("/overview");
});

test("an absolute off-origin URL is rejected", () => {
  expect(sanitizeNext("https://evil.com")).toBe("/overview");
});
...
test("the control-character bypass is rejected (parser strips the tab → off-origin)", () => {
  ...
  expect(sanitizeNext("/\t//evil.com")).toBe("/overview");
  expect(sanitizeNext("/\n//evil.com")).toBe("/overview");
  expect(sanitizeNext("/\\evil.com")).toBe("/overview");
});

test("a non-path scheme is rejected", () => {
  expect(sanitizeNext("javascript:alert(1)")).toBe("/overview");
});

test("missing / empty / array input falls back", () => {
  expect(sanitizeNext(undefined)).toBe("/overview");
  expect(sanitizeNext("")).toBe("/overview");
