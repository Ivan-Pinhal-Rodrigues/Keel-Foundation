...
- **plan-03** — `src/server/modules/approval/` and `src/server/modules/change/` ship the change-management lifecycle and the generic approvals engine. New error `ConflictError` (`src/server/policy/errors.ts`, → 409) for a state clash — an approval step that is not current, a request already resolved. `src/server/modules/approval/`: `openApprovalRequest` / `getApprovalState` / `recordDecision` / `cancelRequest` / `listApprovalsForActor` (`service.ts`) plus `currentStep` / `resolveRequestStatus` / `overrideActionFor` (`state.ts`, pure). The engine is subject-agnostic — an `ApprovalRequest` targets `(subjectType, subjectId)` (`"change"` in v1), steps are ordered and hat-routed, and `recordDecision` runs the current-step invariant then the hat check then the SoD check (creator self-approving needs a `>= 20`-char `overrideJustification`, which flags the decision `isSingleApproverOverride` and writes a second `approval.override` audit event). `approval.*` audit events (`request_opened` / `step_approved` / `step_rejected` / `override` / `request_resolved` / `request_cancelled`) all sit on `subjectType: "ApprovalRequest"`; the `APPROVAL_NEEDED` / `STATUS_CHANGED` notifications are keyed on the subject (the change) so a subject feed and the deep-links match. `src/server/modules/change/`: `CHANGE_STAGES` / `CHANGE_TRANSITIONS` / `assertTransition` / `gateFor` (`state.ts`, pure — safe from a client component) — the seven-stage machine and the per-stage exit-gate predicates (spec 03 §3: `rollbackPlan` required at `assessing → approval`, a future window at `scheduled`, the EMERGENCY carve-outs at `approval` / `pir`). `serializeChange` / `serializeChangeListItem` / `changeStatusLabel` (`serialize.ts`) — INTERNAL-ONLY, no allowlist and no guest branch (a guest is denied before the serializer runs); the drawer payload carries the `stepper` in `LifecycleStepperProps` shape. Service (`service.ts`): `createChange`, `createChangeFromDemand` (idempotent on `Change.originatingDemandId @unique` — a repeat returns the existing change with no second audit), `editChange` (also audits `change.risk_assessed` / `change.rollback_plan_set`; locked once `IMPLEMENTING`+), `linkIncident` (`CAUSED_BY` / `FIXES`, idempotent), `listChanges`, `getChangeForActor`, `advanceChange` (one forward edge per stage, gated by `gateFor`), `scheduleChange` (sets the window and folds in the `APPROVAL → SCHEDULED` enter for an approved or EMERGENCY change), `rollbackChange` (`IMPLEMENTING → ROLLED_BACK`), `recordPir` (record-of-fact, `.create` only), `submitForApproval` (opens a 1-step `change.standard` or 2-step `change.high_risk` request and moves `ASSESSING → APPROVAL`), `changeApprovalContext` (the narrow read the approve route needs). The demand→change wire: `convertDemand` (`src/server/modules/demand/service.ts`) authorises `demand.convert` (BUSINESS/TECHNICAL_APPROVER), calls `createChangeFromDemand`, and on the creating call moves the demand `APPROVED → CONVERTED` with a `demand.converted` audit; `guestStatusLabel` now follows the linked change — a `CONVERTED` demand reads "In progress" until the change is `CLOSED`, then "Delivered" (`getDemandForActor` includes `convertedToChange: { status }`). SoD override action names: `change.approve.technical.override` / `change.approve.business.override`. UI: `ApprovalPanel` component; the `/changes` register (+ scheduled-windows list, Changes nav) and the `/approvals` page (+ nav). Consumed by plan-04 (dashboards: approvals-waiting, change calendar, demand pipeline; the "Delivered" / "fixed" guest wires).
- **plan-04** — notifications read side + the internal dashboard + the portal shell. `src/server/modules/notify/read.ts`: `listNotifications(actor, { unread?, kind? }, client?)` / `unreadCount(actor, client?)` / `markRead(actor, { ids } | { all: true }, client?)` / `listFailedEmails(client?)` (the `{ count, recent }` email-health read — the one `EmailOutbox` query, kept here so `overview/service.ts` stays Prisma-free) plus pure `hrefFor(subjectType, subjectId, actorKind)` and `serializeNotification`. Every notification query is `userId: actor.id`-scoped and nothing else — a `Notification` belongs to one user, so there is no client scope and no `authorize` beyond the route's `notification.view.own` formality; `serializeNotification` reads the `{ summary, subjectType, subjectId }` payload `emitNotification` writes. Routes: `GET /api/notifications` (`?unread` / `?kind=`) → `{ notifications, unreadCount }`; `POST /api/notifications/read` (`{ ids }` or `{ all: true }`) → `{ updated }`. `NotificationBell` component (`@radix-ui/react-popover` added to deps) — unread badge, mark-all-read, in both the internal top bar and the portal top bar. `/notifications` page — full list, internal and guest (guest deep-links land on `/portal/...`). Dashboard reads: `countDemandsByStatus(client?)` (`demand/service.ts`, `Record<DemandStatus, number>`), `listOverdueIncidents(client?)` (`incident/service.ts`, all not-RESOLVED/CLOSED past `dueAt`), `listScheduledWindows(client?)` (`change/service.ts`, SCHEDULED with a window inside 14 days), `recentEvents(limit, client?)` (`audit/read.ts`, newest-first, each with an `auditActionLabel`). `buildOverview(actor, client?)` (`overview/service.ts`) + `GET /api/overview` — internal-only (`requireInternal` → `ForbiddenError`/403 before any read), composes MODULE READS ONLY: no `@prisma/client` / `@/server/db/client` import, no `$queryRaw` / `$executeRaw` (the `OverviewClient` type is borrowed structurally from a read's signature); an `overview.integration.test.ts` pins both the composed payload and that architecture rule. Payload: `tiles` / `myQueue` (incident + owned-change + hat-routed triaging-demand rows, `href` = `/incidents?open=` / `/changes?open=` / `/demands?open=`, overdue sorts first) / `approvals` (`href` `/approvals`) / `activity` / `funnel` (submitted→converted, REJECTED never a stage) / `windows` / `emailFailures`. `/overview` is the internal home — `/` and the post-login redirect both land there. `Me` (`auth/current.ts`) gains `clientName: string | null` for the portal top-bar org name. Portal shell: the `/portal` top bar (org name + bell + account menu) and the unified `/portal/submit` two-tab page (demand | incident), retiring `/portal/incidents/new`; an unread-comment dot on portal request and incident cards. Consumed by plan-05 (deploy — health / readiness already exist; the dashboard is a post-deploy smoke-test target).
